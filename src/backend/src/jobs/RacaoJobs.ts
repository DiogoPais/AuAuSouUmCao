...
export const iniciarJobsDeRacao = () => {
  // Função que alimenta os cães
  const alimentarCaes = async (refeicao: string) => {
  ...
  cron.schedule('0 12 * * *', () => alimentarCaes('Almoço'));
  
  // '0 20 * * *' = Correr todos os dias às 20:00
  cron.schedule('0 20 * * *', () => alimentarCaes('Jantar'));
